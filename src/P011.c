...
int Largest_product_in_a_grid() 
{
	...
	for (int i = 0; i < 17; i++)
	{
		for (int j = 0; j < 17; j++)
		{
			...
			int product = 0;
			if(product1 > product2) product = product1;
...
int Is_Bounds(int x, int y){ 
	if(y>=0 && y< sizeof(grid)/sizeof(int) && x>=0 && x < sizeof(grid[y])/sizeof(int)){
		return 1;
	}
	return 0;
}

int product(int x,int y,int dx,int dy){
	if(Is_Bounds(x + (CONSECUTIVE - 1) * dx,y + (CONSECUTIVE - 1) * dy)){
		return 0;
	}
	int product = 1;
	for(int i=0;i < CONSECUTIVE; i++,x+=dx,y+=dy) product *= grid[y][x];
	return product;
}
